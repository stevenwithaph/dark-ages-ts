...
export class MaxRectsPacker<T extends IRectangle = Rectangle> {
  ...
  public addArray(rects: T[]) {
    if (!this.options.tag || this.options.exclusiveTag) {
      ...
      rects.sort((a, b) => {
        const aTag =
          a.data && a.data.tag ? a.data.tag : a.tag ? a.tag : undefined;
        const bTag =
          b.data && b.data.tag ? b.data.tag : b.tag ? b.tag : undefined;
        return bTag === undefined
          ? -1
          : aTag === undefined
            ? 1
            : bTag > aTag
        ...
        .find((bin, binIndex) => {
          ...
          for (let i = currentIdx; i < rects.length; i++) {
            ...
            if (tag !== currentTag) {
              ...
              currentTag = tag;
              // do addArray()
              this.sort(rects.slice(currentIdx, i), this.options.logic).forEach(
                (r) => bin.add(r)
              );
              currentIdx = i;
            ...
            if (tag === undefined) {
              // do addArray()
              this.sort(rects.slice(i), this.options.logic).forEach((r) =>
                this.add(r)
              );
          ...
          this.sort(rects.slice(currentIdx), this.options.logic).forEach((r) =>
            bin.add(r)
          );
          return true;
        });
