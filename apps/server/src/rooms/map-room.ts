...
import { EntityTypes } from '../scene/entity-types';
...
export class MapRoom extends Room {
  ...
  constructor() {
    ...
    this.monster.identity.networkId = this.id.next();
    this.monster.setPosition(25, 47);
    this.scene.addChild(this.monster);
  }

  async loadMap(id: number) {
    this.mapData = new Uint16Array((await fs.readFile(`./data/map-data/lod${id}.map`)).buffer);
    this.width = 100;
    this.height = 100;
  }
